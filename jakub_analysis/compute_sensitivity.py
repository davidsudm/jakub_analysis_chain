...
def sigma_lima(N_on, N_off, alpha):
    if alpha <= 0: 
        raise AttributeError('ERROR in sigma_lima: alpha must be >0')
    sigma_lima = np.ones_like(N_on, dtype=float) * np.nan
    good = np.logical_and(N_on > 0, N_off > 0)
    in_log1 = (1+alpha)/alpha * N_on[good] / (N_on[good] + N_off[good])
    in_log2 = (1+alpha) * (N_off[good] / (N_on[good] + N_off[good]))
    sigma_sqared = 2 * (
        N_on[good] * np.log(in_log1) + N_off[good] * np.log(in_log2)
    )
    sigma_sqared[sigma_sqared<0] = 0
    sigma_lima[good] = np.sqrt(sigma_sqared)
    return sigma_lima
...
def sensitivity_comp(N_gamma, N_proton, alpha, mid_energy):
    ...
    sensitivity_5sig = np.ones_like(sigma) * np.nan
    sensitivity_5sig[sigma != 0] = 5/sigma[sigma != 0] * crab_hegra_dNdE(mid_energy[sigma != 0])

    # pozadavek na N_gamma > 10
    sensitivity_10gamma = np.ones_like(N_gamma, dtype=float) * np.inf
    sensitivity_10gamma[N_gamma>0] = 10/N_gamma[N_gamma>0] * crab_hegra_dNdE(mid_energy[N_gamma>0])

    # pozadavek na N_gamma > 0.05 * N_bkg
    sensitivity_005bkg = np.ones_like(N_gamma, dtype=float) * np.inf
    sensitivity_005bkg[N_gamma>0] = 0.05 * (N_proton[N_gamma>0]*alpha) / N_gamma[N_gamma>0] * crab_hegra_dNdE(mid_energy[N_gamma>0])
...
def compute_energy_resolution(relative_error_energy, e_true, energy_bins, plot='resolution_energy.png', txtfile='resolution_energy.txt'):
    ...
    for i in range(len(energy_bins)-1):
        ...
        if np.sum(mask) > 10:
            ...
            h1=np.quantile(relative_error_energy[mask], 0.15865)   # 50 – (68.27 / 2) = 15.865
            h2=np.quantile(relative_error_energy[mask], 0.84135)   # 50 + (68.27 / 2) = 84.135
            e_resol68[i] = (h2-h1)/2
        else:
            e_resol68[i] = np.nan
            e_bias[i] = np.nan
...
if __name__ == '__main__':
    ...
    energy_bins = np.logspace(np.log10(0.01), np.log10(100), 21)   # 5 bins per decade
    ...
    compute_resolution_off_axis(param_gamma_diffuse['error'], param_gamma_diffuse['mc_energy'], param_gamma_diffuse['offaxis_angle'], energy_bins, offaxis_angle_bins=[0, 0.5, 1, 2, 3], plot='resolution_68_offaxis.png', txtfile='resolution_68_offaxis.txt')
    ...
    plt.step(energy_bins_proton_mid, param_proton_energy_hist_qc, 'r-.', label='p. after quality cuts ({:.2f} Hz)'.format(np.sum(param_proton_energy_hist_qc)))
    plt.step(energy_bins_proton_mid, param_proton_energy_hist_gh, 'r:', label='p. after gh separation({:.2f} Hz)'.format(np.sum(param_proton_energy_hist_gh)))
    plt.step(energy_bins_proton_mid, param_proton_energy_hist_reco, 'r-', label=r'p. after $\Theta^2$ cuts({:.2f} Hz)'.format(np.sum(param_proton_energy_hist_reco)))
    #plt.step(energy_bins_gamma_mid, param_gamma_energy_hist_dl2, '--', label='reconstructed')
    plt.step(energy_bins_gamma_mid, param_gamma_energy_hist_qc, 'b-.', label=r'$\gamma$ after quality cuts({:.2f} Hz)'.format(np.sum(param_gamma_energy_hist_qc)))
    plt.step(energy_bins_gamma_mid, param_gamma_energy_hist_gh ,'b:', label=r'$\gamma$ after gh separation({:.2f} Hz)'.format(np.sum(param_gamma_energy_hist_gh)))
    plt.step(energy_bins_gamma_mid, param_gamma_energy_hist_reco, 'b-', label=r'$\gamma$ after $\Theta^2$ cuts({:.2f} Hz)'.format(np.sum(param_gamma_energy_hist_reco)))
    plt.ylabel('Event Rate [Hz]')
